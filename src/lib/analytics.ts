...
export const IS_OWNER = !!localStorage.getItem('weightcast_owner_mode')

const devParam = new URLSearchParams(window.location.search).get('dev')
if (devParam === 'owner') localStorage.setItem('weightcast_owner_mode', '1')
if (devParam === 'off')   localStorage.removeItem('weightcast_owner_mode')
...
let _deviceId = localStorage.getItem('weightcast_device_id')
if (!_deviceId) {
  _deviceId = 'dev_' + Math.random().toString(36).slice(2, 9) + Date.now().toString(36)
  localStorage.setItem('weightcast_device_id', _deviceId)
}
